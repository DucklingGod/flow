...
- The delete flow clears local plan/history storage, the disabled encrypted sync queue, and the separate market-data cache. Files already downloaded by the user are outside the browser and cannot be remotely deleted.
...
- ADR-002 specifies future optional-account consent, client-owned keys, unrecoverable-data disclosure, role boundaries, and conflict handling. A test-enabled adapter can persist at most 25 opaque encrypted queue items in IndexedDB and complete local deletion purges them, but production `cloudSync=false` prevents enqueueing; no account identifier, membership, recovery factor, key envelope, plaintext sync payload, or remote journal is persisted or transmitted in the current release.

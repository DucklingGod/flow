...
- [x] `P9-T01d` เพิ่ม IndexedDB persistence สำหรับ encrypted sync queue แบบ opaque และจำกัด 25 รายการ, ลบรายการเมื่อ server ACK, ปฏิเสธ queue state/ID ที่ไม่ตรง contract, ไม่ fallback ลง localStorage และรวม queue ใน local-delete purge; production `cloudSync=false` จึงยังไม่มีการ enqueue หรือส่งข้อมูลจริง
...
- [x] `P10-T06` เตรียม repository delivery controls: GitHub CI, dependency updates, pull-request checklist, release-boundary scan และ initial-publish handoff; public `origin/main` ถูก push และ hosted `verify` ผ่านสำหรับ `3426ab7` ที่ run `31161122786` แล้ว โดยยังต้องตั้งค่า branch protection/review enforcement บน GitHub
